...
struct pm_state
{
	...
	dblist_t* output;  //list of patterns //mallock list// char*
	struct pm_state * fail; //failure function, pointer to the state that we should go when there is no transition  
	dblist_t* _transitions; //list of edges all the next passes  //regular Node
};
...
dblist_t* pm_fsm_search(pm_state_t *,unsigned char *,size_t);
